...
assert.equal((await call('/api/projects/'+id,'PUT',{project:{...project,site_id:'S202103'},revision:1,shared:true})).status,400);
...
console.log('PASS: Daejeon-only model, N:N floors, route references, server persistence, owner isolation, public read-only, revision conflicts, upload access, estimate validation.');db.close();
