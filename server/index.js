...
export default {async fetch(request,env){const u=new URL(request.url),p=u.pathname;
 ...
 try{
   ...
   if(p==='/api/projects'&&request.method==='POST'){
     const raw=await bytes(request,2_000_000),input=raw.length?JSON.parse(new TextDecoder().decode(raw)):{},body=validateProject(input.project||sampleProject());const id=crypto.randomUUID(),secret=token||crypto.randomUUID(),hash=await digest(secret),now=new Date().toISOString();
   ...
   if(request.method==='PUT'){
     const input=JSON.parse(new TextDecoder().decode(await bytes(request,2_000_000))),body=validateProject(input.project);if(!Number.isInteger(input.revision)||typeof input.shared!=='boolean')return json({error:'리비전·공개 설정 오류'},400);
